...
import math
from recognizer_helper import *

# constants (same as js code from https://depts.washington.edu/acelab/proj/dollar/index.html)
NUM_UNISTROKES = 5
NUM_POINTS = 64
SQUARE_SIZE = 250.0
ORIGIN = Point(0, 0)
...
ANGLE_RANGE = Helper.deg2rad(45.0)
ANGLE_PRECISION = Helper.deg2rad(2.0)


class Unistroke:
    def __init__(self, name, points):
        self.name = name
        self.points = Helper.resample(points, NUM_POINTS)
        radians = Helper.indicative_angle(self.points)
        self.points = Helper.rotate_by(self.points, -radians)
        self.points = Helper.scale_to(self.points, SQUARE_SIZE)
        self.points = Helper.translate_to(self.points, ORIGIN)
...
class DollarRecognizer:
    ...
    def __init__(self):
        ...
        self.unistrokes.append(Unistroke("right square bracket", [Point(112, 138), Point(112, 136), Point(115, 136), Point(118, 137), Point(120, 136), Point(123, 136), Point(125, 136), Point(128, 136), Point(131, 136), Point(134, 135), Point(137, 135), Point(140, 134), Point(143, 133), Point(145, 132), Point(147, 132), Point(149, 132), Point(152, 132), Point(153, 134), Point(154, 137), Point(155, 141), Point(156, 144), Point(
            157, 152), Point(158, 161), Point(160, 170), Point(162, 182), Point(164, 192), Point(166, 200), Point(167, 209), Point(168, 214), Point(168, 216), Point(169, 221), Point(169, 223), Point(169, 228), Point(169, 231), Point(166, 233), Point(164, 234), Point(161, 235), Point(155, 236), Point(147, 235), Point(140, 233), Point(131, 233), Point(124, 233), Point(117, 235), Point(114, 238), Point(112, 238)]))
    ...
    def recognize(self, points):
        ...
        for i in range(len(self.unistrokes)):
            ...
            # distance between the candidate and the unistroke to check
            d = Helper.distance_at_best_angle(
                candidate.points, self.unistrokes[i], -ANGLE_RANGE, ANGLE_RANGE, ANGLE_PRECISION)

            # if the distance is less than the best distance so far, update best distance and best unistroke index
            if d < b:
                b = d
                u = i

        # no result -> return "No match" with score 0.0
        if u == -1:
            return Result("No match", 0.0)

        # return the name of the best unistroke and the score (1.0 - distance / half diagonal)
        return Result(self.unistrokes[u].name, 1.0 - b / HALF_DIAGONAL)
